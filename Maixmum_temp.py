...
tmax_data_subset = tmax_var[start_index:end_index+1, lat_indices, lon_indices]

# Compute the trend of the tmax data
def trend(data):
...
trend_vals = np.apply_along_axis(trend, axis=0, arr=tmax_data_subset)
...
contour = ax.pcolormesh(lons, lats, trend_data,vmin = -0.008,vmax = 0.008, cmap=cmap,
                      transform=ccrs.PlateCarree())

# Add a colorbar
cbar = plt.colorbar(contour, ax=ax)
cbar.ax.set_ylabel('Trend (°C/year)')

# Add a title and show the plot
plt.title('Maximum Temperature Trends (1990-2020)')
plt.show()
